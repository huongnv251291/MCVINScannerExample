...
OCR_PPredictor::infer_ocr(cv::Mat &origin,int max_size_len, int run_det, int run_cls, int run_rec) {
  ...
  if (origin.cols < 960) {
        ofset_x=(int)(960-origin.cols)/2.0;
    }
  if (origin.rows < 960) {
        ofset_y=(int)(960-origin.rows)/2.0;
    }
  cv:: Mat origin_p;
  copyMakeBorder(origin,origin_p,ofset_y,ofset_y,ofset_x,ofset_x,cv::BORDER_CONSTANT,cv::Scalar(0));
  
  std::vector<OCRPredictResult> ocr_results;
  if(run_det){
    infer_det(origin_p, max_size_len, ocr_results);
  }
  if(run_rec){
    ...
    for(int i = 0; i < ocr_results.size();i++) {
      infer_rec(origin_p, run_cls, ocr_results[i]);
    }
  }else if(run_cls){
    ClsPredictResult cls_res = infer_cls(origin_p);
    OCRPredictResult res;
